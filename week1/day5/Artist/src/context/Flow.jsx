          ...
          transaction(listingIndex: Int) {
            ...
            prepare(account: AuthAccount) {
              let marketRef = getAccount(${process.env.REACT_APP_ARTIST_CONTRACT_HOST_ACCOUNT})
                .getCapability(/public/LocalArtistMarket)
                .borrow<&{LocalArtistMarket.MarketInterface}>() ?? panic("Couldn't borrow market reference")
          ...
          transaction(listingIndex: Int) {
            ...
            prepare(account: AuthAccount) {
              ...
              let vaultRef = account.borrow<&FungibleToken.Vault>(from: /storage/flowTokenVault) 
                  ?? panic("Couldn't borrow vault reference")

              let tokenVault <- vaultRef.withdraw(amount: listing.price)
